...
import { MORGUE_MIN_QUERY, searchMorgue } from '../game/morgue';
import { isHardPuzzle, isPrimerPuzzle } from '../utils/edition';
...
export const CaseFileModal: React.FC<CaseFileModalProps> = ({
  ...
  const searching = query.trim().length >= MORGUE_MIN_QUERY;
  const clippings = useMemo(() => searchMorgue(puzzles, solvedPuzzleIds, query), [puzzles, solvedPuzzleIds, query]);
  ...
  return (
    ...
    >
        {dossiers.length > 0 && (
          <div className="shrink-0 px-3 py-2 bg-[#d9d0bc] border-b border-stone-500/60">
            <label className="flex items-center gap-2">
              <span className="font-typewriter font-black text-xs uppercase tracking-widest text-stone-800">Morgue</span>
              <input
                type="search"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Search your decoded pages"
                aria-label="Search the morgue: your decoded pages"
                className="flex-1 min-w-0 px-2 py-1 border border-stone-600 bg-[#f6f1e7] font-typewriter text-sm text-stone-950"
              />
            </label>
          </div>
        )}
        {searching ? (
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 bg-newsprint space-y-3" aria-live="polite">
            {clippings.length === 0 ? (
              <p className="font-treatise italic text-stone-600 text-sm">
                Nothing in the morgue. Only pages you have decoded are on file.
              </p>
            ) : (
              clippings.map((clip) => (
                <article key={clip.id} className="evidence-slip border border-stone-700 px-3 pt-2 pb-3">
                  <p className="font-typewriter font-black text-xs uppercase tracking-widest text-stone-800">
                    {isPrimerPuzzle(clip) ? 'Primer' : `Day ${clip.editionNumber} · ${isHardPuzzle(clip) ? 'Night Extra' : 'Morning Edition'}`}
                  </p>
                  <p className="mt-1 font-headline font-black text-sm text-stone-950">{clip.headline}</p>
                  <p className="mt-1 font-typewriter text-sm text-stone-900 underline decoration-stone-500">
                    &ldquo;{clip.originalText}&rdquo;
                  </p>
                </article>
              ))
            )}
